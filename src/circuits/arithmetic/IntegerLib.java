...
public class IntegerLib<T> extends CircuitLib<T> implements ArithmeticLib<T> {
	...
	public int width;
	public void resetRegister(SecureArray<T> reg, int loc) {
		reg.trivialOram.content[loc] = zeros(reg.trivialOram.dataSize);
	}
	...
	public void multiplyMips(T[] x, T[] y, T[] high, T[] low) {
		T[] res = multiplyMipsInternal(x, y, SIGNAL_ZERO);
		//T[][] ret = env.newTArray(2, 0);
		low = Arrays.copyOf(res, 32);
		high = Arrays.copyOfRange(res, 32,64);
	...
	public T[] multiplyMips(T[] x, T[] y) {
		T[] res = multiplyMipsInternal(x, y, SIGNAL_ZERO);
		return Arrays.copyOf(res, 32);
	}
